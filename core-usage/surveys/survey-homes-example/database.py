...
class Database:
    ...
    def __init__(self, path:str = 'database.db', date_user_id = time.mktime((2025, 8, 10, 0, 0, 0, 0, 0, 0))):
    ...
    def allows_respondent_to_continue(self, responses:dict):
        ...
        query = "SELECT quota - actual FROM Quotas WHERE sq1_age_group = ? AND sq2_gender = ? AND sq3_continent = ?"
    ...
    def update_quota(self, respondent_id):
        ...
        query_quotas = "UPDATE Quotas SET actual = actual + 1 WHERE sq1_age_group = ? AND sq2_gender = ? AND sq3_continent = ?"
        params = (responses[0])
